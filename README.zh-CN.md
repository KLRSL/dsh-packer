...
版本 **v0.2.2** · MIT License

**v0.2.1（2026-09-05）**：Config Packer 面板 UI 按「骨架/血肉/呼吸」设计语言重构——neutralSurface 底 + 白色卡片（max-width 860 居中、圆角 16）、4/8px 栅格留白、150ms 克制动效；配色严格取 dsh-fuse default 令牌（--pk-* 变量，零硬编码色值）；差异报表四列新增计数徽章 + 语义色点（新增=绿/变更=橙/相同与跳过=灰）；隐私风险默认警示橙、仅分享拦截用错误红；交互路径（勾选/策略/预览/打包/恢复）零改动，host 测试 24/24 全绿。

**v0.2.0（2026-09-05）**：安全加固与评审修复——
① 隐私扫描修复：合并后的规则数组真正参与循环（此前部署者注入的 `personalPatterns` 被静默跳过）；
② 恢复前对每个源文件做 manifest SHA-256 校验（**fail-closed**：不匹配直接拒绝，不再复制污染文件）；
③ 恢复路径 containment：包内 `../` 越界（zip-slip / 篡改清单）一律拒绝；
④ 结构化配置（JSON/YAML）禁用 append 合并（拼接即损坏），提示改用覆盖或手工合并。
**兼容性**：DeepSeek Harness ≥ 0.1.1-rc.2。
