...
[English](README.md) · Agent 配置打包器（Configuration Packer for DeepSeek Harness）

> **v0.2.2** · MIT License · Node.js ≥ 22.19.0 · DeepSeek Harness ≥ 0.1.1-rc.2

**dsh-packer** 是 **DeepSeek Harness (DSH)** 的「Agent 配置打包器」插件：把本地 Agent 资产按模块打包成标准 zip，用于两种场景：

- **迁移** —— 换机器或重装后，把整套环境完整搬过去。
- **分享** —— 把 Skills 等资产交给别人（自动过滤敏感信息）。
...
然后把 `dsh-packer` 加入 profile 的 `dsh.profile.bundles` 列表，重启 DSH。
...
| 版本 | 日期 | 类型 | 要点 |
|---|---|---|---|
| **v0.2.2** | 2026-09-05 | 适配 / UI | 适配 DSH 0.1.2-rc.1；管理面板按「骨架/血肉/呼吸」设计语言定制——打包工作流布局（阶段流程条 / 器材面板 / diff 色带）+ 橙琥珀青品牌色（打包迁移）+ 深色适配（DSH 主题 `--dsw-alias-*` 跟随，双通道探测） |
| **v0.2.1** | 2026-09-05 | UI 重构 | Config Packer 面板 UI 重构——neutralSurface 底 + 白色卡片（max-width 860 居中、圆角 16）、4/8px 栅格、150ms 克制动效；配色取自 dsh-fuse default 令牌（`--pk-*` 变量零硬编码）；差异报表四列计数徽章 + 语义色点（新增=绿 / 变更=橙 / 相同与跳过=灰）；隐私风险默认警示橙 |
| **v0.2.0** | 2026-09-05 | 安全加固 | ①隐私扫描修复：合并后的个人规则真正参与循环（此前部署者注入的 `personalPatterns` 被静默跳过）；②恢复前对每个源文件做 manifest SHA-256 校验（fail-closed，不匹配拒绝）；③恢复路径 containment（zip-slip / 篡改清单的 `../` 越界拒绝）；④结构化配置（JSON/YAML）禁用 append 合并（会损坏），改为覆盖或手工合并 |
